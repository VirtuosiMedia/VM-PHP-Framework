...
class Vm_External_CssMin {
	...
	*      following spaces.
	* @return      stringMinified stylesheet definitions
	*/
	public static function minify($css, $options = "remove-last-semicolon") {
		...
		if (in_array("preserve-urls", $options)){
			...
			$css = preg_replace_callback("/url\s*\((.*)\)/siU", "cssMinEncodeUrl", $css);
		}
		// Remove comments
	...
	public static function toArray($css, $options = ""){
		...
		if (count($items[0]) > 0){
			for ($i = 0; $i < $c = count($items[0]); $i++){
				...
				$r[] = array(
					"keys"=>cssMinArrayClean($keys),
					"styles"=>cssMinArrayClean($styles)
				);
			}
		}
		return $r;
...
* @return array
*/
function cssMinArrayClean(array $array){
	$r = array();
	$c = count($v);
	if (cssMinArrayIsAssoc($array)){
...
* @return boolean
*/
function cssMinArrayIsAssoc($array){
	if (!is_array($array)){
		return false;
	} else {
...
* @return string
*/
function cssMinEncodeUrl($match){
