 ...
 */
class Suite_Model_Security_Settings extends Vm_Model {
	...
	protected function compileData(){
		...
		$magicQuotes = ($magicQuotesClass == 'pass') ? 'Magic Quotes is disabled.' : 'Magic Quotes must be disabled.';
		$displayErrorsClass = (ini_get('display_errors')) ? 'pass' : 'warning';
		$displayErrors = ($displayErrorsClass == 'pass') ? 'display_errors is disabled. Enable for development only.' : 'display_errors is enabled. Disable for production use.';
		$allowUrlIncludeClass = (ini_get('allow_url_include')) ? 'pass' : 'warning';
		$allowUrlInclude = ($allowUrlIncludeClass == 'pass') ? 'allow_url_include is disabled.' : 'allow_url_include is enabled and can be a security risk.';
		$allowUrlFopenClass = (ini_get('allow_url_fopen ')) ? 'pass' : 'warning';
		$allowUrlFopen = ($allowUrlFopenClass == 'pass') ? 'allow_url_fopen is disabled.' : 'allow_url_fopen is enabled and can be a security risk.';
		$registerGlobalsClass = (ini_get('register_globals')) ? 'pass' : 'fail';
		$registerGlobals = ($registerGlobalsClass == 'pass') ? 'register_globals is disabled.' : 'register_globals is enabled and is a security risk.';
		$exposePhpClass = (ini_get('expose_php')) ? 'warning' : 'pass';
		$exposePhp = ($exposePhpClass == 'pass') ? 'expose_php is disabled.' : 'expose_php is enabled and may be a security risk.';
