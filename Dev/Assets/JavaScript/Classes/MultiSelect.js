...
Modified by:
Virtuosi Media Inc for our test framework
*/
var MultiSelect = new Class({
	...
	options: {
		...
		containerClass: 'selectMulti', 	// element container CSS class
	...
	initialize: function(selector, formId, options) {
		// set options
	...
	buildMenu: function(element) {
		...
		var containers = element.getElements(self.options.containers);
		
		// list container
		var list = new Element('ul', {
			'id':element.get('id')+'List',
			'class':'selectList',
			'styles': { display: 'none' },
			'events': {
		...
		boxes.each(function(box, i) {
			...
			var label = labels[i];
			new Element('li', {
				'class': box.get('checked') ? self.options.itemSelectedClass + ' '+containers[i].get('rel') : containers[i].get('rel'),
				'events': {
					'mouseenter': function() {
						if (self.active === true) {
		...
		element.empty().adopt([monitor]);
		
		var coordinates = monitor.getCoordinates();
//*
		list.setStyles({
			'position':'absolute',
...
//*/		
	...
	changeItemState: function(item, checkbox, monitor) {
		if (item.hasClass(this.options.itemSelectedClass)) {
			...
			checkbox.set('checked', 'checked').focus();
		}
	...
	changeMonitorValue: function(list) {
		var textLength = list.getElements(this.options.boxes).filter(function(box) {
			return box.get('checked');
		}).length; 
		var monitorText = (textLength > 1) ? this.options.monitorText : this.options.monitorText.substring(0, this.options.monitorText.length - 1);
		var text = ((textLength == 0)&&(this.options.initialText)) ? this.options.initialText : textLength+' '+monitorText;
		return text;
	}, 
	...
	toggleMenu: function(toggle, monitor, list) {
		if (toggle == 'open') {
			...
			var closeList = this.elements.getElement('div.monitor').getParent('div').get('id')+'List';
			$(closeList).setStyle('display', 'none');
